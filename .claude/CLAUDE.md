...
- **Default output folder** is derived from `#filePath` (resolved at compile time to `main.swift`'s absolute path on the machine that built it), not the process's working directory. This is intentional — it's stable regardless of where/how the binary is launched. A user-chosen override lives in `UserDefaults` (`OutputFolder` key).
